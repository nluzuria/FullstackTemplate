...
// import { CommenttModel } from "../mongo/model/comment";
...
async function getSampleBlogPosts() {
...
export async function createSampleDataIfDbEmpty() {
  try {
    ...
    let existingPosts = await getSampleBlogPosts();

    if (existingPosts.length > 0) {
      console.log(
        `✔ ${existingPosts.length} example posts found, not Adding anymore!`
      );
      return;
    } else {
      await createSampleBlogPosts(samplePosts);
